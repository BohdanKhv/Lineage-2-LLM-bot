      ...
      this.LoginClient.connect()
        .then(() => {
          ...
          this.LoginClient.once("PacketReceived:PlayOk", () => {
            // Standard L2: close the login connection right after PlayOk, then
            // connect to the game server. The login server keeps a grace window
            // so the game can validate the session.
            setTimeout(() => {
          ...
          this.GameClient.once("PacketReceived:CharSelected", () => {
            // Interlude: send EnterWorld directly (skip HF-only Manor/KeyMapping Ex packets).
            this.GameClient.sendPacket(new EnterWorld()).catch((e) => reject("Enter world fail." + e));
          });
