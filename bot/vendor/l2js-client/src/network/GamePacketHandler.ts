...
export default class GamePacketHandler implements IPacketHandler<GameClient> {
  ...
  handlePacket(data: Uint8Array, client: GameClient): ReceivablePacket {
    ...
    try {
      // Interlude (protocol 746) S->C opcode map, extracted from the elmore
      // gameserver.jar serverpackets. (The stock lib map was High Five.)
      switch (opcode) {
        case 0x00:
          // 0x00 is the crypt-init KeyPacket, only before crypt bootstrap.
          rpk = client.CryptBootstrapped ? (undefined as never) : new Packets.KeyPacket();
          break;
        case 0x01:
